...
<body style="background-repeat: no-repeat; background-position: center; background-size: cover; background-image: url('<?php echo base_url(); ?>assets/user_template/images/bg-01.jpg');">
		...
		<?php
		if($fetch_data_users->num_rows()>0){	
		?>
<div class="parent">
  	<div class="child">		
		  <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:right;">
			<form action="<?php echo base_url(); ?>Projects/all">
				<button class="login100-form-btn mybtn">
					ALL PROJECTS (<span> <?php $this->load->model('user_model'); 
					echo $this->user_model->totalProjects();?>)</span>
				</button>
			</form>
		</div>
	</div>
	 	...
	 	<div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:center;">
			<form action="<?php echo base_url(); ?>User/all">
				<button class="login100-form-btn mybtn" style="background-color:rgba(256,256,256,0.3);">
					ALL USERS (<span> <?php $this->load->model('user_model'); 
					echo $this->user_model->totalUsers();?>)</span>
				</button>
			</form>
		</div>
	</div>
  	<div class="child">		
	   <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;">
			<form action="<?php echo base_url(); ?>Projects/requests">
				<button class="login100-form-btn mybtn">
					REQUESTS (<span> <?php $this->load->model('user_model'); 
					echo $this->user_model->totalRequests();?>)</span>
				</button>
			</form>
		</div>
	</div>
  	<div class="child">		
	    <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:left;">
			<form action="<?php echo base_url(); ?>Email/all">
				<button class="login100-form-btn mybtn">
					MAILBOX (<span> <?php $this->load->model('user_model'); 
					echo $this->user_model->totalEmails();?>)</span>
				</button>
			</form>
			</div>
		</div>
		...
		<div class="wrap-login200 p-l-15 p-r-15 p-t-15 p-b-0" style="margin-left:auto; margin-right:auto; width:1200px;">
			<div class="p-l-15 p-r-15 p-t-15 p-b-30 "style="text-align:center;font-size:18px; font-weight:700;" >
				<form action="<?php echo base_url();?>User/search" method="post">
				 <div class="row">
				  ...
				  <br>				
				<?php
				if($fetch_data_users->num_rows()>4){	
				?>
			...
			foreach($fetch_data_users->result() as $row){
			?>
		            <tr style="font-size:17px; font-weight:600;">
						...
						<?php 
							...
							echo $this->user_model->projectsNumber($row->id_user);
						?></td>
						<td> 
				 		<a class="deleteProject" id="<?php echo $row->id_user?>"href="#"><button type="button"class="btn btn-circle mybtn3" style="border:2px solid #C82333;color:#C82333;height:40px; width:40px; margin-left:-10px;border-radius:20px;"><i class="fa fa-trash"></i></button></a>
					</td>
						</tr>
			<?php
			}
 	...
 	<div class="child">		
  	...
  	<div class="child">		
		...
		<div class="wrap-login200 p-l-30 p-r-30 p-t-30 p-b-40" style="margin-left:auto; z-index:10; margin-right:auto; width:1200px;">
			...
			?>
				<?php if($search_found==''){?>
								<div class="p-l-20 p-r-20 p-t-20 p-b-20"style="text-align:center;font-size:20px; font-weight:700;"> There are no users in database. </div> 
