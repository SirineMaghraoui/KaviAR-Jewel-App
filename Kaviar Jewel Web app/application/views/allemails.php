...
transition: all 0.2s;
	  ...
	  .mybtn5:hover{
		background-color:rgba(0,166,183,0.4);
	  }		
...
<body style="background-repeat: no-repeat; background-position: center; background-size: cover; background-image: url('<?php echo base_url(); ?>assets/user_template/images/bg-01.jpg');">
		...
		<?php
		if($fetch_data_emails->num_rows()>0){	
		?>
<div class="parent">
  	<div class="child">		
		  <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:right;">
			<form action="<?php echo base_url(); ?>Projects/all">
				<button class="login100-form-btn mybtn">
					ALL PROJECTS (<span> <?php $this->load->model('email_model'); 
					echo $this->email_model->totalProjects();?>)</span>
				</button>
			</form>
		</div>
	</div>
 	<div class="child">		
	 	<div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:center;">
			<form action="<?php echo base_url(); ?>User/all">
				<button class="login100-form-btn mybtn">
					ALL USERS (<span> <?php $this->load->model('email_model'); 
					echo $this->email_model->totalUsers();?>)</span>
				</button>
			</form>
		</div>
	</div>
  	<div class="child">		
	   <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;">
			<form action="<?php echo base_url(); ?>Projects/requests">
				<button class="login100-form-btn mybtn">
					REQUESTS (<span> <?php $this->load->model('email_model'); 
					echo $this->email_model->totalRequests();?>)</span>
				</button>
			</form>
		</div>
	</div>
  	<div class="child">		
	    <div class="wrap-login100-form-btn" style="width:60%;margin-top:-20px;float:left;">
			<form action="<?php echo base_url(); ?>Email/all">
				<button class="login100-form-btn mybtn"style="background-color:rgba(256,256,256,0.3);">
					MAILBOX (<span> <?php $this->load->model('email_model'); 
					echo $this->email_model->totalEmails();?>)</span>
				</button>
			</form>
			</div>
		</div>
		...
		<div class="wrap-login200 p-l-15 p-r-15 p-t-15 p-b-0" style="margin-left:auto; margin-right:auto; width:1200px;">
			<div class="p-l-15 p-r-15 p-t-15 p-b-30 "style="text-align:center;font-size:18px; font-weight:700;" >
				<form action="<?php echo base_url();?>Email/search" method="post">
				 <div class="row">
					<div class="col-lg-12">
				  ...
				  <br>
				<?php
				if($fetch_data_emails->num_rows()>4){	
				?>
			...
			foreach($fetch_data_emails->result() as $row){
			?>
		            <tr style="font-size:17px; font-weight:600;">
						<td><?php echo $row->id_email?></td>
						<td><?php 
						$this->load->model('email_model');
						echo $this->email_model->getUsername($row->id_user);
						?></td>
                		<td><?php echo $row->subject ?></td>
						   <td> <?php echo $row->date ?></td> 
						<td> 
							<a style="color:white;" href="<?php echo base_url();?>Email/view/<?php echo $row->id_user?>/<?php echo $row->id_email?>"><button type="button"  style="color:#00A6B7; border:2px solid #00A6B7;background-color:none;margin-right:20px;border-radius:20px; height:40px; width:40px;" class="btn btn-circle mybtn5"><i class="fa fa-envelope-open "></i> </button></a>
				 		<a class="deleteProject" id="<?php echo $row->id_email?>"href="#"><button type="button" class="btn btn-circle mybtn3" style="border:2px solid #C82333;color:#C82333;height:40px; width:40px; margin-left:-10px;border-radius:20px;"><i class="fa fa-trash"></i></button></a>
						</tr>
			<?php
			}
			?>
 	...
 	<div class="child">		
  	...
  	<div class="child">		
		...
		<div class="wrap-login200 p-l-30 p-r-30 p-t-30 p-b-40" style="margin-left:auto; z-index:10; margin-right:auto; width:1200px;">
			...
			?>
				<?php if($search_found==''){?>
								<div class="p-l-20 p-r-20 p-t-20 p-b-20"style="text-align:center;font-size:20px; font-weight:700;"> Inbox is empty. </div> 
			...
			$(document).ready(function(){
				...
				$('.deleteProject').click(function(){
					...
					if(confirm("Are you sure you want to delete this email?")){
						window.location="<?php echo base_url();?>Email/deleteEmail/"+id;
					}else{
						return false;
					}
				})
